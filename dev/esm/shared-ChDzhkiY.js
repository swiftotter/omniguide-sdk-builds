import { g as getPreviewApiUrl } from "./shared-mg2rmv2z.js";
...
function getLastRecommendations() {
  ...
  return window[LAST_KEY] ?? null;
}
function onRecommendations(handler) {
  if (typeof window === "undefined") return () => {
  };
  const listener = (e) => {
  ...
  window.addEventListener(RECOMMENDATIONS_EVENT, listener);
  return () => window.removeEventListener(RECOMMENDATIONS_EVENT, listener);
}
...
class PlatformNotInitializedError extends OmniguideError {
  constructor(platformName) {
    super(
      "PLATFORM_NOT_INITIALIZED",
      `Platform adapter${platformName ? ` (${platformName})` : ""} is not initialized`,
      { context: { platformName } }
    );
    this.name = "PlatformNotInitializedError";
  }
}
class ConfigurationError extends OmniguideError {
  constructor(message, context) {
    super("CONFIGURATION_ERROR", message, { context });
    this.name = "ConfigurationError";
  }
}
class StorageError extends OmniguideError {
  constructor(message, cause) {
    super("STORAGE_ERROR", message, { cause });
    this.name = "StorageError";
  }
}
class SdkParseError extends OmniguideError {
  constructor(endpoint, zodError) {
    super("SDK_PARSE_ERROR", `[Omniguide] Failed to parse ${endpoint} response`, {
      context: { endpoint }
    });
...
function isOmniguideError(error) {
  return error instanceof OmniguideError;
}
function wrapError(error, defaultMessage = "An unknown error occurred") {
  if (error instanceof OmniguideError) {
    return error;
  }
  if (error instanceof Error) {
    return new OmniguideError("UNKNOWN_ERROR", error.message, { cause: error });
  }
  return new OmniguideError("UNKNOWN_ERROR", defaultMessage, {
    context: { originalError: String(error) }
  });
}
let override = null;
function setCurrentPageOverride(url) {
  override = url;
}
...
const SourcesMessageSchema = objectType({
...
const DiscoveryQuestionMessageSchema = objectType({
...
const ErrorMessageSchema = objectType({
  type: literalType("error"),
  message: stringType().optional(),
  error: stringType().optional(),
  code: stringType().optional()
}).passthrough();
const PingMessageSchema = objectType({
  type: literalType("ping").or(literalType("pong"))
}).passthrough();
const FitEvaluationMessageSchema = objectType({
...
const QuestionMessageSchema = objectType({
...
function safeJsonParse(text) {
  try {
    const sanitized = text.replace(/:\s*Infinity\s*([,}\]])/g, ":null$1").replace(/:\s*-Infinity\s*([,}\]])/g, ":null$1").replace(/:\s*NaN\s*([,}\]])/g, ":null$1");
...
function parseAndValidateMessage(text) {
  const data = safeJsonParse(text);
  if (data === null) return null;
  return validateWebSocketMessage(data);
}
function isStreamChunk(msg) {
  return msg.type === "stream_chunk" || msg.type === "answer";
}
function isSessionIdMessage(msg) {
  return msg.type === "session_id";
}
function isSourcesMessage(msg) {
  return msg.type === "sources";
}
function isDiscoveryQuestion(msg) {
  return msg.type === "discovery_question" || msg.type === "intent_question" || msg.type === "clarification_question";
}
...
function createAPIWithRetry(config) {
...
const WS_ENDPOINTS = {
  SEARCH: "/ws/search",
  PRODUCT_RECOMMENDATIONS: "/ws/product-recommendations",
  CATEGORY_RECOMMENDATIONS: "/ws/category-recommendations"
};
const ERROR_MESSAGES = {
  NETWORK: "I'm having trouble connecting to the search service. Please check your internet connection and try again.",
  SERVER: "Our search service is temporarily unavailable. Please try again in a moment.",
  TIMEOUT: "The search is taking longer than expected. Please try again.",
  PARSE: "I encountered an error while processing the results. Please try rephrasing your question.",
  GENERIC: "Something went wrong with your search. Please try again.",
  SESSION_EXPIRED: "Session expired. Please refresh the page.",
  NOT_CONNECTED: "Not connected to the server. Please try again."
};
const TIMEOUTS = {
  API_REQUEST: 1e4,
  WEBSOCKET_CONNECTION: 1e4,
  WEBSOCKET_HEARTBEAT: 5e3
};
const LATENCY = {
...
function getPlatformAdapter() {
  ...
  if (!adapter.isInitialized()) {
    logger.warn("Platform adapter not initialized");
  }
  return adapter;
}
function requirePlatformAdapter() {
  const adapter = platformRegistry.get();
  if (!adapter.isInitialized()) {
    throw new PlatformNotInitializedError(adapter.getPlatformName());
  }
  return adapter;
}
...
class SessionStorage {
  constructor(storage, storageKey = DEFAULT_STORAGE_KEYS.sessionId) {
    this.sessionId = null;
    this.storage = storage;
    this.storageKey = storageKey;
    this.sessionId = this.storage.getItem(this.storageKey);
  }
  /**
   * Get current session ID
   */
  ...
  /**
   * Update session ID (called when server provides canonical ID)
   */
  update(newSessionId) {
    ...
    if (this.sessionId !== newSessionId) {
      ...
      this.storage.setItem(this.storageKey, newSessionId);
    }
  }
  /**
   * Clear session ID
...
function isDiscoveryQuestionId(questionId) {
  const idStr = String(questionId);
  return idStr.startsWith("discovery_") || idStr.startsWith("dynamic_");
}
function toDiscoveryId(id) {
  const idStr = String(id);
  if (idStr.startsWith("discovery_") || idStr.startsWith("dynamic_") || idStr.startsWith("intent_")) {
    return idStr;
  }
  return `discovery_${id}`;
}
class ResponseTimer {
...
function cleanText(raw = "") {
  ...
  text = removeMd(text, { listUnicodeChar: "", useImgAltText: true });
  text = text.replace(/\s+/g, " ").trim();
  return text;
}
function shortenText(text, limit = MAX_SUMMARY_CHARS) {
  if (!text) return "";
...
function clearSessionId(websiteId) {
  ensureEntry(websiteId).sessionId = null;
}
...
function clearSession(websiteId) {
  const registry = getRegistry();
  delete registry[websiteId];
  featureStatusSubscribers.delete(websiteId);
  writeLocalStorage(conversationStorageKey(websiteId), null);
}
...
const log$2 = createScopedLogger("ConsentService");
...
class ConsentService {
  ...
  async ensureInitialized(sessionId) {
    ...
    try {
      ...
      if (!ok) {
        log$2.warn("Consent server returned non-200 — keeping cookie-derived consent");
      }
    } catch (e) {
      log$2.warn("Consent server unreachable — keeping cookie-derived consent:", e);
    }
  ...
  async refresh(sessionId) {
    ...
    if (scopeJson !== this.lastScopeJson) {
      ...
      try {
        const ok = await this.sendScopeToServer(sessionId, scope);
        if (!ok) {
          log$2.warn("Consent refresh server returned non-200 — keeping cookie-derived consent");
        }
      } catch (e) {
        log$2.warn("Consent refresh server unreachable — keeping cookie-derived consent:", e);
      }
    }
    this.initialized = true;
    this.dispatchChange();
  ...
  canSendAnalytics() {
    ...
    if (can !== this.lastCanSendAnalytics) {
      ...
      if (!can) {
        log$2.debug(`canSendAnalytics=false (websiteConsent=${this._websiteConsent}, omniguideConsent=${sharedOmniguideConsent})`);
      }
    }
    return can;
  }
...
const log$1 = createScopedLogger("EventService");
...
class EventService {
  ...
  track(eventName, data = {}) {
    ...
    if (this.queue.length > this.maxQueueSize) {
      ...
      log$1.warn(`Queue exceeded max size, dropped ${removed.length} oldest events`);
    }
    this.saveQueue();
  ...
  tryKick() {
    ...
    this.sendBatch(eventDataToSend).then(() => {
      ...
      log$1.warn("Kick fetch failed, requeuing:", error);
  ...
  async flush() {
    if (this.queue.length === 0 || this.isFlushing) return;
    if (!this.sessionId) {
      log$1.debug("Flush deferred: no sessionId yet");
      return;
    }
    this.isFlushing = true;
    const batch = this.queue.splice(0, this.batchSize);
    ...
    try {
      ...
      log$1.error("Flush failed:", error);
      for (const queuedEvent of batch) {
        queuedEvent.attempts++;
        if (queuedEvent.attempts < this.maxRetries) {
          this.queue.push(queuedEvent);
        } else {
          log$1.error(`Event "${queuedEvent.data["event"]}" dropped after ${this.maxRetries} attempts`);
        }
      }
      this.saveQueue();
    } finally {
  ...
  async sendEvents(params) {
    ...
    try {
      ...
      log$1.error("Failed to send event batch:", error);
    }
  }
  /**
  ...
  loadQueue() {
    ...
    try {
      ...
      if (valid.length > 0) {
        ...
        log$1.debug(`Loaded ${valid.length} event(s) from storage`);
      }
...
let instance = null;
let consentInstance = null;
function registerEventService(service) {
  instance = service;
}
...
function updateEventContext(updates) {
  if (!instance) return;
  instance.updateContext(updates);
}
function registerConsentService(service) {
  consentInstance = service;
}
function getConsentState() {
  if (!consentInstance) return null;
  return consentInstance.getState();
}
const log = createScopedLogger("DataLayerBridge");
const POLL_INTERVAL_MS = 1e3;
const SOURCES = {
  elevar: { key: "ElevarDataLayer" },
  // The filter is the GTM source's, not the bridge's: an Elevar store that
  // ever names an event `gtm.something` must still be forwarded.
  gtm: { key: "dataLayer", skip: (name) => name.startsWith("gtm.") }
};
...
const ENVELOPE_KEYS = ["event", "timestamp", "message_id"];
const SDK_ORIGIN_MARKER = "omniguide_sdk";
...
function detachPayload(payload) {
  try {
    return structuredClone(payload);
  } catch (error) {
    log.warn("structuredClone failed; forwarding the vendor object by reference:", error);
    return payload;
  }
}
...
function activeSources() {
  const specs = [];
  if (activeConfig == null ? void 0 : activeConfig.elevar) specs.push(SOURCES.elevar);
  if (activeConfig == null ? void 0 : activeConfig.gtm) specs.push(SOURCES.gtm);
  return specs;
}
function emitPageViewOnce(service) {
  if (!(activeConfig == null ? void 0 : activeConfig.pageView) || pageViewSent) return;
  try {
    service.track("page_view", {
      // Sanitised like every other analytics URL in the SDK: a shopper can
      // land on a path carrying a reset token or a session id in the query,
      // and page_location would otherwise ship it verbatim.
    ...
    log.error("page_view could not be forwarded; continuing with sources:", error);
  }
}
function drainSource(spec, service, budget) {
  var _a;
  let left = budget;
  for (const entry of readArray(spec.key)) {
    if (left <= 0) return 0;
    ...
    try {
      ...
      if (!parsed) continue;
      if ((_a = spec.skip) == null ? void 0 : _a.call(spec, parsed.name)) continue;
      left--;
      service.track(parsed.name, detachPayload(parsed.payload));
    } catch (error) {
      log.error(`Skipping unforwardable entry on ${spec.key}:`, error);
    }
  }
  return left;
}
function drain(unbounded = false) {
  const service = getEventService();
  if (!service) return;
  emitPageViewOnce(service);
  let budget = unbounded ? Number.POSITIVE_INFINITY : MAX_FORWARDS_PER_TICK;
  for (const spec of activeSources()) {
    budget = drainSource(spec, service, budget);
    if (budget <= 0) return;
  }
}
function tick(unbounded = false) {
  try {
    drain(unbounded);
  } catch (error) {
...
function startDataLayerBridge(config) {
  if (typeof window === "undefined") return () => {
  };
  if (!config.elevar && !config.gtm && !config.pageView) return () => {
  };
...
export {
  APIError as $,
  API_ENDPOINTS as A,
  extractSkusFromMarkdown as B,
  createResponseTimer as C,
  ChatWebSocket as D,
  ERROR_MESSAGES as E,
  FLOW_STATES as F,
  filterEmptyContent as G,
  filterRedundantContent as H,
  getConversationId as I,
  setConversationId as J,
  getSessionStart as K,
  LocalStorageAdapter as L,
  getApiBaseUrl as M,
  NullPlatformAdapter as N,
  DEFAULT_STORAGE_KEYS as O,
  clearSessionId as P,
  SdkParseError as Q,
  RestSessionResponseSchema as R,
  SDK_ORIGIN_MARKER as S,
  TYPEAHEAD_SECTION_ORDER as T,
  objectType as U,
  booleanType as V,
  numberType as W,
  arrayType as X,
  ...
  unknownType as _,
  safeHref as a,
  requirePlatformAdapter as a$,
  APITimeoutError as a0,
  API_URLS as a1,
  AnswerOptionSchema as a2,
  ApiClient as a3,
  BaseWebSocket as a4,
  CategorySchema as a5,
  ConfigurationError as a6,
  ConnectionTimeoutError as a7,
  ConsentService as a8,
  ConversationIdMessageSchema as a9,
  TYPEAHEAD_ENDPOINT as aA,
  WS_ENDPOINTS as aB,
  WebSocketError as aC,
  WebSocketMessageSchema as aD,
  cleanText as aE,
  clearSession as aF,
  createAPIWithRetry as aG,
  dedupeByUrl as aH,
  getConsentState as aI,
  getEventService as aJ,
  getLastRecommendations as aK,
  getPageContext as aL,
  getPlatformAdapter as aM,
  getWebSocketBaseUrl as aN,
  isDiscoveryQuestion as aO,
  isDiscoveryQuestionId as aP,
  isErrorMessage as aQ,
  isLocalhost as aR,
  isOmniguideError as aS,
  isPingMessage as aT,
  isSafeNavigationUrl as aU,
  isSessionIdMessage as aV,
  isSourcesMessage as aW,
  isStreamChunk as aX,
  normalizeFeedbackResponse as aY,
  onRecommendations as aZ,
  parseAndValidateMessage as a_,
  DiscoveryQuestionMessageSchema as aa,
  ErrorMessageSchema as ab,
  EventService as ac,
  FeedbackAPI as ad,
  FitEvaluationMessageSchema as ae,
  LATENCY as af,
  MaxReconnectsError as ag,
  MemoryStorageAdapter as ah,
  OmniguideError as ai,
  PingMessageSchema as aj,
  PlatformNotInitializedError as ak,
  ProductSchema as al,
  QuestionMessageSchema as am,
  RECOMMENDATIONS_EVENT as an,
  RECONNECTION as ao,
  ResponseTimer as ap,
  RestDiscoveryAnswerSchema as aq,
  RestDiscoveryQuestionSchema as ar,
  RestFeedbackResponseSchema as as,
  RestQuestionsResponseSchema as at,
  SessionIdMessageSchema as au,
  SessionStorage as av,
  SourcesMessageSchema as aw,
  StorageError as ax,
  StreamChunkSchema as ay,
  TIMEOUTS as az,
  setSessionId as b,
  safeJsonParse as b0,
  setCurrentPageOverride as b1,
  shortenText as b2,
  startDataLayerBridge as b3,
  toDiscoveryId as b4,
  trackEvent as b5,
  updateEventContext as b6,
  validateMessage as b7,
  validateWebSocketMessage as b8,
  wrapError as b9,
  enumType as ba,
  literalType as bb,
  createScopedLogger as c,
  setFeatureStatus as d,
  setSessionStart as e,
  formatPrice as f,
  getCurrentPage as g,
  emitRecommendations as h,
  normalizeQuestions as i,
  getSessionId as j,
  AnsweredIntentsStorage as k,
  logger as l,
  getFeatureStatus as m,
  normalizeSessionResponse as n,
  onFeatureStatusChange as o,
  createPlatformAdapter as p,
  capturePageContext as q,
  platformRegistry as r,
  sanitizeUrl as s,
  transformSummary as t,
  createConsentService as u,
  createEventService as v,
  registerEventService as w,
  createFeedbackAPI as x,
  registerConsentService as y,
  fetchTypeaheadSearch as z
};
//# sourceMappingURL=shared-ChDzhkiY.js.map
